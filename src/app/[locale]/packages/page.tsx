...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    slug: "/packages/",
    title: page.packagesPage.title,
    description: page.packagesPage.intro,
    keywords: ["website pricing", "web packages", "landing page", "corporate website"]
  });
}
...
export default async function PackagesPage({ params }: PageProps) {
  ...
  const breadcrumbs = breadcrumbSchema([
    ...
    { name: page.packagesPage.title, url: absoluteUrl(`/${locale}/packages/`) }
  ]);
